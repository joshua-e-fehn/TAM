#!/usr/bin/env python3
"""
Failure Position Plotter

This script plots track boundaries, collisions, and track boundary violations
...
Usage:
    python3 plot_failure_positions.py [--mode MODE] [--output OUTPUT_DIR]
    
Example:
    python3 plot_failure_positions.py --mode single_car_with_obstacle
    python3 plot_failure_positions.py --mode multi_car
    python3 plot_failure_positions.py --mode combined
"""
...
# Failure type markers - professional academic style
FAILURE_MARKERS = {
    'collision': {
        'marker': 's',  # Square - solid, professional
        'size': 80,
        'edgecolor': 'black',
        'linewidth': 1.2,
        'label': 'Collision'
    ...
    'trackbounds': {
        'marker': '^',  # Triangle - distinct, clean
        'size': 90,
        'edgecolor': 'black',
        'linewidth': 1.2,
...
def plot_failure_positions(ax, failures: List[RaceRecord], converter: FrenetConverter,
                           failure_type: str):
    """Plot failure positions for each planner"""
    stats = {}

    for planner, style in sorted(PLANNER_STYLES.items(), key=lambda x: x[1]['order']):
        planner_failures = [r for r in failures if r.planner ==
                            planner and r.race_end == failure_type]

        if len(planner_failures) > 0:
            cart_coords = []
            for record in planner_failures:
                try:
            ...
            if cart_coords:
                ...
                marker_style = FAILURE_MARKERS[failure_type]
...
def create_legend_elements():
    """Create legend elements for planners and failure types"""
    elements = []
    ...
    # Add failure types
    for failure_type, marker_style in FAILURE_MARKERS.items():
        elements.append(
...
def plot_single_map_mode(map_name: str, waypoints_data: dict, failures: List[RaceRecord],
    ...
    # Plot failures
    collision_stats = plot_failure_positions(
        ax, failures, converter, 'collision')
    trackbound_stats = plot_failure_positions(
        ax, failures, converter, 'trackbounds')

    # Title and labels
    mode_display = 'Single-Car With Obstacle' if mode == 'single_car_with_obstacle' else 'Multi-Car'
    ax.set_title(f"{config['name']} - {mode_display}\nCollision & Track Boundary Violation Positions",
    ...
    total_collisions = sum(collision_stats.values())
    total_trackbounds = sum(trackbound_stats.values())
    stats_text = f"Collisions: {total_collisions}\nTrack Bounds: {total_trackbounds}\nTotal: {total_collisions + total_trackbounds}"
    ...
    # Save figures
    filename = f"failure_positions_{map_name}_{mode}"
    ...
    return collision_stats, trackbound_stats
...
def plot_combined_map(map_name: str, waypoints_data: dict,
    ...
    for records, mode_title, ax in datasets:
        # Filter for this map and failure types
        map_failures = [r for r in records if r.map_name == map_name
                        and r.race_end in ['collision', 'trackbounds']]
        ...
        # Plot failures
        collision_stats = plot_failure_positions(
            ax, map_failures, converter, 'collision')
        trackbound_stats = plot_failure_positions(
            ax, map_failures, converter, 'trackbounds')

        all_stats.append((collision_stats, trackbound_stats))
        ...
        total_col = sum(collision_stats.values())
        total_tb = sum(trackbound_stats.values())
        stats_text = f"Collisions: {total_col}\nTrack Bounds: {total_tb}"
    ...
    # Main title
    fig.suptitle(f"{config['name']} - Failure Position Analysis",
    ...
    # Save figures
    filename = f"failure_positions_{map_name}_combined"
    save_figure(fig, output_dir, filename)
...
def plot_all_failures_single_figure(records: List[RaceRecord], waypoints_data_f: dict,
    ...
    for map_name, mode, waypoints_data, title, ax in configurations:
        # Filter records
        map_failures = [r for r in records if r.map_name == map_name
                        and r.mode == mode and r.race_end in ['collision', 'trackbounds']]
        ...
        total_col = sum(collision_stats.values())
        total_tb = sum(trackbound_stats.values())
        ax.text(0.04, 0.96, f"Col: {total_col} | TB: {total_tb}",
                transform=ax.transAxes, fontsize=10, verticalalignment='top',
                bbox=dict(boxstyle='round,pad=0.3', facecolor='wheat',
                          edgecolor='black', alpha=0.9))

    fig.suptitle('Failure Position Analysis - All Scenarios',
                 fontsize=18, fontweight='bold', y=0.98)
    ...
    # Save figures
    filename = "failure_positions_all_scenarios"
    save_figure(fig, output_dir, filename)
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot failure positions on track maps")
    ...
    if args.mode == 'all' and len(maps_to_process) == 2:
        print("\n🎨 Creating comprehensive 2x2 failure analysis plot...")
        plot_all_failures_single_figure(
            records,
        ...
        for map_name in maps_to_process:
            ...
            if args.mode == 'combined':
                ...
                for mode in modes_to_process:
                    failures = filter_records(records, map_name=map_name, mode=mode,
                                              race_ends=['collision', 'trackbounds'])

                    if len(failures) == 0:
                        print(f"   No failures found for {mode} on {map_name}")
                        continue

                    print(f"   Plotting {mode}: {len(failures)} failures")
                    plot_single_map_mode(map_name, waypoints_data, failures,
