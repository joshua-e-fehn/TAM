...
class MarinaMapParser:
    def __init__(self, csv_file: str, output_map_name: str = "marina", scale_factor: float = 0.1, width_multiplier: float = 2.0):
        """
        Initialize the Marina map parser with proper dimensionless scaling.

        DIMENSIONLESS SCALING THEORY:
        ============================
        To maintain dynamic similarity between full-scale and scaled vehicles,
        we must preserve key dimensionless numbers:

        1. FROUDE NUMBER: Fr = v²/(g·L)
           - Relates inertial forces to gravitational forces
           - Must remain constant for dynamic similarity

        2. LATERAL ACCELERATION COEFFICIENT: v²·κ/g
           - Determines cornering behavior
           - Must remain constant for same driving dynamics

        SCALING LAWS (for scale factor λ):
        =================================
        - Length:        L_scaled = L_original × λ
        - Time:          t_scaled = t_original × √λ (Froude scaling)
        - Velocity:      v_scaled = v_original × √λ (preserves Froude number)
        - Acceleration:  a_scaled = a_original (same gravitational environment)
        - Curvature:     κ_scaled = κ_original / λ
        - Frequency:     f_scaled = f_original / √λ

        For λ = 0.1 (10% scale):
        - Velocities scale by √0.1 ≈ 0.316 (not 0.1!)
        - 100 km/h becomes ~32 km/h (not 10 km/h)
        - Accelerations remain the same
        - Lap times scale by √λ ≈ 0.316

        This ensures the scaled vehicle exhibits the same relative dynamics
        as the full-scale vehicle.
    ...
    def create_centerline_waypoint(self, values: List[str], waypoint_id: int) -> Dict:
        ...
        # Proper curvature scaling: κ_scaled = κ_original / λ
        kappa_radpm = float(
        ...
        # Apply proper dimensionless scaling to velocity (√λ not λ)
        velocity_scale = self.scale_factor**0.5  # √λ for Froude scaling
        # 30% speed reduction + proper scale factor
        vx_mps = rl_vx_mps * 0.7 * velocity_scale
        # 50% acceleration reduction (no scaling needed - same gravity)
    ...
    def create_sp_waypoint(self, values: List[str], waypoint_id: int) -> Dict:
        ...
        # Apply proper dimensionless scaling to velocity (√λ not λ)
        velocity_scale = self.scale_factor**0.5  # √λ for Froude scaling
        # 15% speed reduction + proper scale factor
        vx_mps = rl_vx_mps * 0.85 * velocity_scale
        # 20% acceleration reduction (no scaling needed - same gravity)
        ax_mps2 = rl_ax_mps2 * 0.8
    ...
    def scale_waypoints(self, waypoints: List[Dict]) -> List[Dict]:
        """Scale waypoints using proper dimensionless scaling laws.

        This applies dimensionless scaling to preserve dynamic similarity:
        - Length scales by λ
        - Time scales by √λ (Froude scaling)
        - Velocity scales by √λ (to preserve Froude number)
        - Acceleration unchanged (same gravitational environment)
        - Curvature scales by 1/λ
        
        Should be called AFTER shortest path generation on unscaled data.
        """
        scaled_waypoints = []
        
        # Calculate proper scaling factors based on dimensionless analysis
        length_scale = self.scale_factor  # λ
        time_scale = length_scale**0.5    # √λ (Froude scaling)
        velocity_scale = time_scale       # √λ (preserves v²/(g·L))
        acceleration_scale = 1.0          # unchanged (same gravity)
        curvature_scale = 1.0 / length_scale  # 1/λ

        print(f"\n=== Dimensionless Scaling Analysis ===")
        print(f"Length scale factor (λ): {length_scale}")
        print(f"Time scale factor (√λ): {time_scale:.3f}")
        print(f"Velocity scale factor (√λ): {velocity_scale:.3f}")
        print(f"Acceleration scale factor: {acceleration_scale}")
        print(f"Curvature scale factor (1/λ): {curvature_scale}")
        print("Dimensionless numbers preserved:")
        print(f"  - Froude number: v²/(g·L) = constant")
        print(f"  - Lateral acceleration: v²·κ/g = constant")
        print("======================================\n")

        for wp in waypoints:
            scaled_wp = {
                'id': wp['id'],
                's_m': wp['s_m'] * length_scale,
                'd_m': wp['d_m'] * length_scale,
                'x_m': wp['x_m'] * length_scale,
                'y_m': wp['y_m'] * length_scale,
                # Already has width_multiplier applied
                'd_right': wp['d_right'] * length_scale,
                # Already has width_multiplier applied
                'd_left': wp['d_left'] * length_scale,
                'psi_rad': wp['psi_rad'],  # Angles don't need scaling
                # Proper curvature scaling to maintain dynamic similarity
                'kappa_radpm': wp['kappa_radpm'] * curvature_scale,
                # Proper velocity scaling using Froude similarity (√λ not λ)
                'vx_mps': wp['vx_mps'] * velocity_scale,
                # Acceleration unchanged (same gravitational environment)
                'ax_mps2': wp['ax_mps2'] * acceleration_scale
            }
