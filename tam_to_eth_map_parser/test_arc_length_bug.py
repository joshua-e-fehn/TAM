...
def test_interpolation_bug():
    ...
    print(
        f"   ❌ ERROR: {abs(buggy_waypoints[-1]['s_m'] - actual_waypoints[-1]['s_m']):.3f}m")
    ...
    print(
        f"   Error growth rate: {max_error/actual_waypoints[-1]['s_m']*100:.1f}% of track length")
    ...
    print(
        f"   Position error: {max_error:.1f}m ({max_error/actual_waypoints[-1]['s_m']*100:.1f}%)")
