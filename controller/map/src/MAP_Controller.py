...
class MAP_Controller:
    ...
    def calc_steering_angle(self, L1_point, L1_distance, yaw, lat_e_norm, v):
        ...
        if np.linalg.norm(L1_vector) == 0:
            ...
            eta = np.arcsin(
                np.dot([-np.sin(yaw), np.cos(yaw)], L1_vector)/np.linalg.norm(L1_vector))
        ...
        steering_angle = np.clip(steering_angle, self.curr_steering_angle -
                                 threshold, self.curr_steering_angle + threshold)
    ...
    def calc_L1_point(self, lateral_error):
        ...
        if len(self.waypoint_array_in_map[self.idx_nearest_waypoint:]) > 2:
            ...
            self.curvature_waypoints = np.mean(
                abs(self.waypoint_array_in_map[self.idx_nearest_waypoint:, 5]))
    ...
    def calc_speed_command(self, v, lat_e_norm):
        ...
        la_position = [self.position_in_map[0, 0] + v[0] *
                       adv_ts_sp, self.position_in_map[0, 1] + v[1]*adv_ts_sp]
    ...
    def trailing_controller(self, global_speed):
        ...
        # gap to opponent
        self.gap = (
            self.opponent[0] - self.position_in_map_frenet[0]) % self.track_length
        ...
        self.i_gap = np.clip(self.i_gap + self.gap_error /
                             self.loop_rate, -10, 10)
    ...
    def speed_steer_scaling(self, steer, speed):
        ...
        """
        speed_diff = max(0.1, self.end_scale_speed -
                         self.start_scale_speed)  # to prevent division by zero
        factor = 1 - np.clip((speed - self.start_scale_speed) /
                             (speed_diff), 0.0, 1.0) * self.downscale_factor
        steer *= factor
        return steer
    ...
    def speed_adjust_lat_err(self, global_speed, lat_e_norm):
        ...
        lat_e_norm *= 2
        curv = np.clip(2*(np.mean(self.curvature_waypoints)/0.8) - 2,
                       a_min=0, a_max=1)  # 0.8 ca. max curvature mean

        global_speed *= (1 - lat_e_coeff + lat_e_coeff *
                         np.exp(-lat_e_norm*curv))
        return global_speed
    ...
    def speed_adjust_heading(self, speed_command):
        ...
        if heading_error < np.pi/9:  # 20 degrees error is okay
            ...
            scaler = 1 - 0.5 * heading_error / \
                (np.pi/2)  # scale linearly to 0.5x
        else:
            scaler = 0.5
        self.logger_info(
    ...
    def nearest_waypoint(self, position, waypoints):
        ...
        distances_to_position = np.linalg.norm(
            abs(position_array - waypoints), axis=1)
