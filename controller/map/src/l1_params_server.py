...
def get_racecar_version():
    # Prefer private (~racecar_version) for multi-car setups; fallback to global
    return rospy.get_param('~racecar_version', rospy.get_param('/racecar_version'))
...
def get_yaml_values():
    ...
    # Get and return data
    with open(f"{yaml_path}/config/{get_racecar_version()}/l1_params.yaml", "r") as file:
...
def callback(config, level):
    ...
    config.speed_lookahead_for_steer = round(
        config.speed_lookahead_for_steer * 20) / 20
    # Trailing Controller
