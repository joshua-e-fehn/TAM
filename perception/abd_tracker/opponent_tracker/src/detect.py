...
class Detect:
    ...
    def __init__(self) -> None:
        ...
        # ORIGINAL PARAMS REMOVED BELOW; replaced with namespaced helpers
        def _p(private_name, global_name, default=None):
        ...
        self.laser_frame = _p('~laser_frame', '/laser_frame',
                              f"{self.frame_prefix}laser")
        ...
        # --- Publisher ---
        self.breakpoints_markers_pub = rospy.Publisher(
            'perception/breakpoints_markers', MarkerArray, queue_size=5)
        self.boundaries_pub = rospy.Publisher(
            'perception/detect_bound', Marker, queue_size=5)
        self.obstacles_msg_pub = rospy.Publisher(
            'perception/detection/raw_obstacles', ObstacleArray, queue_size=5)
        self.obstacles_marker_pub = rospy.Publisher(
            'perception/obstacles_markers_new', MarkerArray, queue_size=5)
        if self.measuring:
            self.latency_pub = rospy.Publisher(
                'perception/detection/latency', Float32, queue_size=5)

        # --- Tunable params ---
        self.rate = _p('~rate', '/detect/rate', 30)
        self.lambda_angle = _p('~lambda', '/detect/lambda', 10) * math.pi/180
        self.sigma = _p('~sigma', '/detect/sigma', 0.05)
        self.min_2_points_dist = _p(
            '~min_2_points_dist', '/detect/min_2_points_dist', 0.02)
    ...
    def dyn_param_cb(self, params: Config):
        self.min_obs_size = rospy.get_param(
            'dynamic_tracker_server/min_obs_size', 10)
        self.max_obs_size = rospy.get_param(
            'dynamic_tracker_server/max_obs_size', 0.5)
        self.max_viewing_distance = rospy.get_param(
            'dynamic_tracker_server/max_viewing_distance', 9)
        self.boundaries_inflation = rospy.get_param(
            'dynamic_tracker_server/boundaries_inflation', 0.1)

        self.path_needs_update = True
        param_list = [self.min_obs_size,
                      self.max_obs_size, self.max_viewing_distance]
        print(
            f'[Opponent Detection]: New dyn reconf values recieved: Min size {param_list[0]} [laser points], Max size {param_list[1]} [m], max viewing dist {param_list[2]} [m]')
    ...
    def scans2ObsPointCloud(self):
        ...
        try:
            ...
            rospy.logerr(
                f"[Opponent Detection]: lookup Transform between {self.map_frame} and {self.laser_frame} not possible")
            empty = []
            return empty
        trans, quat = self.tf_listener.lookupTransform(
            self.map_frame, self.laser_frame, lct)
        T = np.array(trans)
        R = tf.transformations.quaternion_matrix(quat)

        angles = np.linspace(self.scans.angle_min,
                             self.scans.angle_max, len(self.scans.ranges))
        ...
        objects_pointcloud_list = [
            [[cloudPoints_list[0][0], cloudPoints_list[0][1]]]]
    ...
    def main(self):
        ...
        while not rospy.is_shutdown():
            ...
            if self.scans is None:
                # Wait briefly for first scan to arrive to avoid NoneType access
                rospy.sleep(0.05)
                rate.sleep()
                continue
            objects_pointcloud_list = self.scans2ObsPointCloud()
            current_obstacles = self.obsPointClouds2obsArray(
                objects_pointcloud_list)
