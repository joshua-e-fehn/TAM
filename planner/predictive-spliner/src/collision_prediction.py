...
class CollisionPredictor:
    ...
    def __init__(self):
        ...
        # Visualization
        self.marker_beginn = self.marker_init(
            a=0.5, r=0.63, g=0.13, b=0.94, id=0)
    ...
    def opponent_trajectory_cb(self, data: OpponentTrajectory):
        # exclude last point (because last point == first point) <- Hopefully this is still the case?
    ...
    def wpnts_updated_cb(self, data: WpntArray):
        ...
        # Should be the same as self.max_s but just in case. Only used for wrap around
    ...
    def marker_init(self, a=1, r=1, g=0, b=0, id=0):
        marker = Marker(header=rospy.Header(
            stamp=rospy.Time.now(), frame_id="map"), id=id, type=Marker.SPHERE)
    ...
    def delete_all(self) -> None:
        empty_marker = Marker(header=rospy.Header(
            stamp=rospy.Time.now(), frame_id="map"), id=0)
        empty_marker.action = Marker.DELETE
        self.marker_pub_beginn.publish(empty_marker)
        empty_marker.id = 1
        self.marker_pub_end.publish(empty_marker)

        empty_obs_arr = ObstacleArray(header=rospy.Header(
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            if len(self.opponent_pos.obstacles) > 0:
                current_obstacles_msg = ObstacleArray(
                    header=rospy.Header(
                        stamp=rospy.Time.now(), frame_id="map"),
            ...
            if len(opponent_pos_copy.obstacles) != 0:
                ...
                s_points_global_array = np.array(
                    [wpnt.s_m for wpnt in self.wpnts_opponent])  # Opponent
                s_points_updated_array = np.array(
                    [wpnt.s_m for wpnt in self.wpnts_updated])  # Ego (updated)
                ...
                for i in range(self.time_steps):
                    ...
                    opponent_approx_indx = np.abs(
                        s_points_global_array - opponent_position % self.max_s_opponent).argmin()  # Opponent
                    ego_approx_indx = np.abs(
                        s_points_updated_array - ego_position % self.max_s_updated).argmin()  # Ego (scaled)

                    # Opponent
                    opponent_speed = self.wpnts_opponent[opponent_approx_indx].proj_vs_mps
                    # Ego (updated)
                    ego_speed = self.wpnts_updated[ego_approx_indx].vx_mps

                    # Interpolate the acceleration
                    acceleration = self.min_a + (self.max_a - self.min_a) / max(
                        [self.max_v - self.min_v, 10e-6]) * (self.max_v - current_ego_speed)

                    if current_ego_speed < ego_speed:
                        ego_position = (
                            ego_position + current_ego_speed * self.dt + 0.5 * acceleration * self.dt**2)
                        opponent_position = (
                            opponent_position + opponent_speed * self.dt)
                ...
                if beginn == True:
                    ...
                    collision_obs = Obstacle(id=0, d_left=d_left, d_right=d_right, d_center=(
                        d_right + d_left)/2, vs=0, vd=0, is_actually_a_gap=False, is_static=False)
                    collision_obs.s_start = beginn_s
                    collision_obs.s_end = end_s
                    collision_obs.s_center = (beginn_s + end_s)/2

                    collision_obs_arr = ObstacleArray(header=rospy.Header(
                        stamp=rospy.Time.now(), frame_id="map"), obstacles=[collision_obs])
                    self.collision_obs_pub.publish(collision_obs_arr)
                    self.expire_counter = 0

                    # Visualize the collision (Watchout for wrap around)
                    position_beginn = self.frenet2glob(
                        [beginn_s % self.max_s_updated], [beginn_d])
                    ...
                    position_end = self.frenet2glob(
                        [end_s % self.max_s_updated], [end_d])
