...
import numpy as np
import rospy
from f110_msgs.msg import (ObstacleArray, OpponentTrajectory,
                           Obstacle, Wpnt, OppWpnt, WpntArray, ProjOppTraj, ProjOppPoint)
...
class GaussianProcessOppTraj(object):
    def __init__(self):
        ...
        rospy.Subscriber('opponent_trajectory', OpponentTrajectory,
                         self.opp_traj_cb)  # testing with Rosbag
    ...
    def get_gp_opp_traj(self):
        ...
        self.opponent_trajectory = rospy.wait_for_message(
            "opponent_trajectory", OpponentTrajectory)
        ...
        while not rospy.is_shutdown():
            ...
            if opp_on_traj == False and len(proj_opp_traj.detections) != 0:
                oppwpnts_list, opp_traj_marker_array = self.predict_opponent_trajectory(
                    ego_s_doublelap=ego_s_doublelap, proj_opp_detections=proj_opp_traj.detections, oppwpnts_list=oppwpnts_list)
    ...
    def predict_opponent_trajectory(self, ego_s_doublelap: list, proj_opp_detections: list, oppwpnts_list: list):
        ...
        for i in range(len(proj_opp_detections)-1):
            if abs(proj_opp_detections[i].s - proj_opp_detections[i+1].s) > self.track_length/2 or around_origin:
                proj_opp_detections[i +
                                    1].s = proj_opp_detections[i+1].s+self.track_length
                around_origin = True
        ...
        first_index = nr_of_detections - \
            min(nr_of_points_to_merge, nr_of_detections)

        opponent_s_sorted = [proj_opp_detections[i].s for i in range(
            first_index, nr_of_detections)]
        opponent_d_sorted = [proj_opp_detections[i].d for i in range(
            first_index, nr_of_detections)]
        opponent_vs_sorted = [proj_opp_detections[i].vs for i in range(
            first_index, nr_of_detections)]
        opponent_vd_sorted = [proj_opp_detections[i].vd for i in range(
            first_index, nr_of_detections)]
        counter = 0
        for i in range(len(oppwpnts_list_doublelap)):
            if oppwpnts_list_doublelap[i].s_m > last_s_of_detections+gap and counter < nr_of_points_to_merge:
                ...
                opponent_vs_sorted.append(
                    oppwpnts_list_doublelap[i].proj_vs_mps)
        ...
        opponent_vs_sorted_reshape = np.array(
            opponent_vs_sorted).reshape(-1, 1)
        opponent_vd_sorted_reshape = np.array(
            opponent_vd_sorted).reshape(-1, 1)
        ...
        resampled_opponent_vd = np.interp(
            s_pred, opponent_s_sorted, opponent_vd_sorted)
