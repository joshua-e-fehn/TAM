...
class SQPAvoidanceNode:
    ...
    def __init__(self):
        ...
        # ROS Parameters
        # Use relative topic for namespacing
        ...
        # Publishers (use relative topics for proper namespacing)
        self.mrks_pub = rospy.Publisher(
            "planner/avoidance/markers_sqp", MarkerArray, queue_size=10)
        self.evasion_pub = rospy.Publisher(
            "planner/avoidance/otwpnts", OTWpntArray, queue_size=10)
        self.merger_pub = rospy.Publisher(
            "planner/avoidance/merger", Float32MultiArray, queue_size=10)
        if self.measure:
            self.measure_pub = rospy.Publisher(
                "planner/pspliner_sqp/latency", Float32, queue_size=10)
    ...
    def obs_perception_cb(self, data: ObstacleArray):
        ...
        self.obs_perception.obstacles = [
            obs for obs in data.obstacles if obs.is_static == True]
    ...
    def gb_cb(self, data: WpntArray):
        self.global_waypoints = np.array(
            [[wpnt.x_m, wpnt.y_m] for wpnt in data.wpnts])

    # Everything is refered to the SCALED global waypoints
    def scaled_wpnts_cb(self, data: WpntArray):
        self.scaled_wpnts = np.array(
            [[wpnt.s_m, wpnt.d_m] for wpnt in data.wpnts])
    ...
    def local_wpnts_cb(self, data: WpntArray):
        self.local_wpnts = np.array([[wpnt.s_m, wpnt.d_m]
    ...
    def loop(self):
        ...
        rospy.wait_for_message(
            "dynamic_sqp_tuner_node/parameter_updates", Config)
        ...
        while not rospy.is_shutdown():
            ...
            considered_obs = []
            for i, obs_item in enumerate(obs.obstacles):
                dist_to_obs = (obs_item.s_start -
                               self.cur_s) % self.scaled_max_s
                traj_dist = abs(obs_item.d_center)
                within_traj_thresh = traj_dist < self.obs_traj_tresh
            ...
            # publish latency
            if self.measure:
                self.measure_pub.publish(
                    Float32(data=time.perf_counter() - start_time))
            self.rate.sleep()
    ...
    def sqp_solver(self, considered_obs: list, cur_s: float):
        ...
        initial_guess_object_end_idx = np.abs(
            self.scaled_wpnts - initial_guess_object.s_end).argmin()
        # Get array of indexes of the global waypoints overlapping with the ROC
        gb_idxs = np.array(range(initial_guess_object_start_idx, initial_guess_object_start_idx + (
            initial_guess_object_end_idx - initial_guess_object_start_idx) % self.scaled_max_idx)) % self.scaled_max_idx
        ...
        start_avoidance = max(
            (min_s_obs_start - self.back_to_raceline_before), cur_s)
        end_avoidance = max_s_obs_end + self.back_to_raceline_after

        # Get a downsampled version for s avoidance points
        s_avoidance = np.linspace(
            start_avoidance, end_avoidance, self.avoidance_resolution)
        ...
        scaled_wpnts_indices = np.array([np.abs(
            self.scaled_wpnts[:, 0] - s % self.scaled_max_s).argmin() for s in s_avoidance])
        # Get the scaled waypoints for every s avoidance point idx
        corresponding_scaled_wpnts = [
            self.scaled_wpnts_msg.wpnts[i] for i in scaled_wpnts_indices]
        ...
        d_right_arr = np.array(
            [wpnt.d_right for wpnt in corresponding_scaled_wpnts])
        track_width = d_left_arr + d_right_arr
        usable_width = track_width - 2 * self.spline_bound_mindist

        # Calculate curvature at each point using numerical differentiation
        # k = (x'y'' - y'x'') / (x'^2 + y'^2)^(3/2)
        x_global_points = np.array(
            [wpnt.x_m for wpnt in corresponding_scaled_wpnts])
        y_global_points = np.array(
            [wpnt.y_m for wpnt in corresponding_scaled_wpnts])
        ...
        self.global_traj_kappas = (
            x_prime*y_prime_prime - y_prime*x_prime_prime) / ((x_prime**2 + y_prime**2)**(3/2))
        ...
        for obs in considered_obs:
            ...
            if obs_idx_start < len(s_avoidance) - 2:  # Sanity check
                if obs.is_static == True or obs_idx_end == obs_idx_start:
                    ...
                    self.obs_downsampled_indices = np.append(
                        self.obs_downsampled_indices, np.arange(obs_idx_start, obs_idx_end + 1))
                    self.obs_downsampled_center_d = np.append(self.obs_downsampled_center_d, np.full(
                        obs_idx_end - obs_idx_start + 1, (obs.d_left + obs.d_right) / 2))
                    self.obs_downsampled_min_dist = np.append(self.obs_downsampled_min_dist, np.full(
                        obs_idx_end - obs_idx_start + 1, (obs.d_left - obs.d_right) / 2 + self.width_car + self.evasion_dist))
                else:
                    indices = np.arange(obs_idx_start, obs_idx_end + 1)
                    self.obs_downsampled_indices = np.append(
                        self.obs_downsampled_indices, indices)
                    opp_wpnts_idx = [np.abs(
                        self.opponent_wpnts_sm - s_avoidance[int(idx)] % self.max_opp_idx).argmin() for idx in indices]
                    d_opp_downsampled_array = np.array(
                        [self.opponent_waypoints[opp_idx].d_m for opp_idx in opp_wpnts_idx])
                    self.obs_downsampled_center_d = np.append(
        ...
        radius_speed = min([clipped_speed, self.wpnts_updated[(
            scaled_wpnts_indices[0]) % self.max_idx_updated].vx_mps])
        ...
        if result.success == True:
            ...
            n_global_avoidance_points = int(
                (end_avoidance - start_avoidance) / self.scaled_delta_s)
            s_array = np.linspace(
                start_avoidance, end_avoidance, n_global_avoidance_points)
            ...
            downsampled_v = np.array(
                [wpnt.vx_mps for wpnt in corresponding_scaled_wpnts])
            evasion_v = np.interp(s_array, s_avoidance, downsampled_v)
            # Create a new evasion waypoint message
            evasion_wpnts_msg = OTWpntArray(header=rospy.Header(
                stamp=rospy.Time.now(), frame_id="map"))
            evasion_wpnts = []
            evasion_wpnts = [Wpnt(id=len(evasion_wpnts), s_m=s, d_m=d, x_m=x, y_m=y, vx_mps=v)
                             for x, y, s, d, v in zip(evasion_x, evasion_y, evasion_s, evasion_d, evasion_v)]
    ...
    def obstacle_constraint(self, d):
        distance_to_obstacle = np.abs(
            d[self.obs_downsampled_indices] - self.obs_downsampled_center_d)
    ...
    def consecutive_points_constraint(self, d):
        ...
        for i in range(len(points) - 1):
            if not ((points[i] > self.obs_downsampled_center_d[i] and points[i+1] > self.obs_downsampled_center_d[i+1]) or
                ...
                # Add a violation as a negative value if the condition is not met
                violations.append(-1)
            else:
                violations.append(1)
        return violations
    ...
    def turning_radius_constraint(self, d):
        ...
        kappa = (self.down_sampled_delta_s * y_prime_prime) / \
            ((self.down_sampled_delta_s ** 2) ** (3/2))
    ...
    def group_objects(self, obstacles: list):
        ...
        initial_guess_object.s_center = (
            initial_guess_object.s_start + initial_guess_object.s_end) / 2
        return initial_guess_object

    def _more_space(self, obstacle: Obstacle, gb_wpnts, gb_idxs):
        left_boundary_mean = np.mean(
            [gb_wpnts[gb_idx].d_left for gb_idx in gb_idxs])
        right_boundary_mean = np.mean(
            [gb_wpnts[gb_idx].d_right for gb_idx in gb_idxs])
    ...
    def visualize_sqp(self, evasion_s, evasion_d, evasion_x, evasion_y, evasion_v):
        ...
        if len(evasion_s) == 0:
            ...
            for i in range(len(evasion_s)):
                mrk = Marker(header=rospy.Header(
                    stamp=rospy.Time.now(), frame_id="map"))
    ...
    def initialize_converter(self) -> bool:
        ...
        converter = FrenetConverter(
            self.global_waypoints[:, 0], self.global_waypoints[:, 1])
