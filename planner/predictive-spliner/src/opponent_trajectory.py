...
class Opponent_Trajectory:
    def __init__(self):
        ...
        # s, d, vs, vd, is_static, is_visible, timestamp
        self.opponent = [0, 0, 0, 0, False, True, 0]
        self.position_in_map_frenet = []  # current position in frenet coordinates
        # add all the points where the opponent was to this list
        self.opponent_trajectory_list = []
        ...
        # Publisher
        self.proj_opponent_trajectory_pub = rospy.Publisher(
            'proj_opponent_trajectory', ProjOppTraj, queue_size=10)
        self.marker_pub = rospy.Publisher(
            'opponent_marker', MarkerArray, queue_size=10)
        # Subscriber
        rospy.Subscriber("perception/obstacles",
                         ObstacleArray, self.obstacle_cb)
        rospy.Subscriber('car_state/odom_frenet', Odometry,
                         self.car_state_frenet_cb)  # car frenet coordinates
        ...
        rospy.Subscriber('opponent_trajectory', OpponentTrajectory,
                         self.opp_traj_cb)  # global waypoints
    ...
    def make_opponent_trajectory(self):
        ...
        # all positive
        bound_right = [wnpt.d_right for wnpt in self.global_wpnts.wpnts]
        # all positive
        bound_left = [wnpt.d_left for wnpt in self.global_wpnts.wpnts]
        # treshold for checking if we are going "backwards" in the s direction
        self.treshold = self.track_length/2
        ...
        while not rospy.is_shutdown():
            ...
            if index is not None and self.lap_count >= 1:
                time_diff = rospy.Time.now().to_sec() - \
                    raw_opponent_traj[index][6]
                # time_diff = 0 #for ROSBAG
                # if the timestamp is older than 0.5 seconds we discard the measurement (variable)
                if time_diff > 0.5:
                    # publish some extra information about the opponent trajectory (variable)
                    if points_in_lap > 5:
                        ...
                        self.proj_opponent_trajectory_pub.publish(
                            projected_opponent_traj)
                        projected_opponent_traj = ProjOppTraj()
                        raw_opponent_traj = np.zeros((number_of_wpnts, 11))
            ...
            opponent = self.find_nearest_dyn_obstacle(
                obstacle_array=obstacle_array, position_in_map_frenet=position_in_map_frenet, track_length=track_length)
            if opponent is not None:
                old_index = index
                index = self.get_index(ego_s_sorted, opponent[0])

                # if the index is the same as the old index we dont update the opponent trajectory (this also gets rid of the delta time = 0 problem in simulation)
                if index != old_index:
                    # s, d, vs, vd, is_static, is_visible, timestamp, s_var, d_var, vs_var, vd_var
                    ...
                    if ignore_datapoint_flag:
                        ...
                        if self.lap_count >= 1 and self.opp_traj_count > 1:
                            consecutive_points_off_opptraj = self.check_if_opponent_is_on_predicted_trajectory(proj_opponent=proj_opponent,
                                                                                                               consecutive_points_off_opptraj=consecutive_points_off_opptraj, opponent_trajectory=self.opponent_trajectory)
                        projected_opponent_traj.detections.append(
                            proj_opponent)
                        if first_point:
                        ...
                        mrk_msg = self._opp_to_marker(
                            proj_opp=proj_opponent, id=marker_count, delete_bool=False)
                        ...
                        marker_count += 1
                    # if the opponent is off the predicted trajectory for more than 5 points we discard the measurement (variable)
                    if consecutive_points_off_opptraj > 5 and traversed_distance < track_length/2:
                        projected_opponent_traj.nrofpoints = points_in_lap
                        projected_opponent_traj.lapcount = self.lap_count
                                ...
                                if index != old_index:
                                    ...
                                    if ignore_datapoint_flag:
                                        ...
                                        projected_opponent_traj.detections.append(
                                            proj_opponent)
                                        if first_point:
                                            projected_opponent_traj.detections.pop(
                                                0)
                                        traversed_distance += trav_dist
                                        first_point = False
                                        points_in_lap += 1
                         ...
                         # checke for a large s gap between consecutive points
                            ...
                            if initial_lap:  # start over from start_index
                                ...
                                if len(projected_opponent_traj.detections) > 0:
                                    index = self.get_index(
                                        ego_s_sorted, projected_opponent_traj.detections[-1].s)
                                    points_in_lap = points_in_lap - start_index
                                    self.lap_count = 0
                                    # reset traversed distance
                                    traversed_distance = (
                                        projected_opponent_traj.detections[-1].s-projected_opponent_traj.detections[0].s) % self.track_length
                                else:
                                    # reset everything
                                    projected_opponent_traj = ProjOppTraj()
                                    raw_opponent_traj = np.zeros(
                                        (number_of_wpnts, 11))
                                    traversed_distance = 0
                                    points_in_lap = 0
                                    self.lap_count = 0
...
######################### HELPER FUNCTIONS######################################
    ...
    def find_nearest_dyn_obstacle(self, obstacle_array: ObstacleArray, position_in_map_frenet: np.ndarray, track_length: float) -> (np.ndarray, Obstacle):
        ...
        if len(obstacle_array.obstacles) > 0 and len(position_in_map_frenet):
            ...
            for obstacle in obstacle_array.obstacles:
                opponent_dist = (obstacle.s_start -
                                 position_in_map_frenet[0]) % track_length
    ...
    def check_if_opponent_is_on_predicted_trajectory(self, proj_opponent: ProjOppPoint, consecutive_points_off_opptraj: int, opponent_trajectory: OpponentTrajectory):
        ...
        if closest_oppwpnt is not None:
            # if the opponent is within 30cm of the predicted trajectory we assume that the prediction is correct (variable)
            if abs(closest_oppwpnt.d_m - proj_opponent.d) < 0.3:
                consecutive_points_off_opptraj = 0
            else:
    ...
    def project_opponent_trajectory(self, old_index: int, index: int, raw_opponent_traj: np.ndarray, proj_opponent_traj: ProjOppTraj, trav_dist: float, track_length: float, first_point: bool = False, bound_left: list = None, bound_right: list = None) -> (float, np.ndarray, bool):
        ...
        discard_flag = False
        if (not first_point):
            diff_s = raw_opponent_traj[index][0] - \
                raw_opponent_traj[old_index][0]
            diff_d = raw_opponent_traj[index][1] - \
                raw_opponent_traj[old_index][1]
            delta_time, discard_flag = self._get_delta_time(
                old_index=old_index, index=index, opponent=raw_opponent_traj)
            ...
            # around the origin
                ...
                if len(proj_opponent_traj.detections) > 0:
                    # do this since track_length is not very accurate (could also ignore this point)
                    opp_vs = proj_opponent_traj.detections[-1].vs
                else:
                    discard_flag = True
            ...
            curr_vel = np.linalg.norm(
                [raw_opponent_traj[index][2], raw_opponent_traj[index][3]])
            old_vel = np.linalg.norm(
                [raw_opponent_traj[old_index][2], raw_opponent_traj[old_index][3]])
            delta_velocity = abs(curr_vel-old_vel)/delta_time
            if not discard_flag and (np.sqrt(opp_vs**2+opp_vd**2) > 15 or delta_velocity > 20):
                discard_flag = True

            # if detections are more than 1m apart (variable)
            if diff_s > 1 and self.lap_count >= 1:
                discard_flag = True
                first_point = True
                publish_flag = True
    ...
    def _opp_to_marker(self, proj_opp: ProjOppPoint, id: int, delete_bool: bool = False) -> Marker:
        ...
        cartesian_position = self.converter.get_cartesian(
            proj_opp.s, proj_opp.d)
