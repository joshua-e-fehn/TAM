...
class UpdateWaypoints:
    ...
    def __init__(self):
        ...
        # Waypoint publisher
        self.wpnts_updated_pub = rospy.Publisher(
            "global_waypoints_updated", WpntArray, queue_size=10)
        self.marker_pub = rospy.Publisher(
            "updated_waypoints_marker", MarkerArray, queue_size=10)
...
### Callbacks ###
        ...
        if self.update_waypoints == True:
            if self.s_points_array.any():
                ...
                ego_approx_indx = np.abs(
                    self.s_points_array - ego_position).argmin()
                ...
                if self.state == "GB_TRACK" and self.gb_track_start_time is not None and time.time() - self.gb_track_start_time >= self.hysteresis_time:
                    self.wpnts_updated_msg.wpnts[ego_approx_indx].vx_mps = ego_speed + \
                        self.speed_offset
                    # First and last waypoint are the same
                    if ego_approx_indx == 0 or ego_approx_indx == (len(self.wpnts_updated_msg.wpnts) - 1):
    ...
    def visualize_waypoints(self):
        ...
        for i in range(len(self.wpnts_updated_msg.wpnts)):
            ...
            marker.scale.x = 0.1
            marker.scale.y = 0.1
            marker.scale.z = self.wpnts_updated_msg.wpnts[i].vx_mps / \
                self.max_speed_scaled
    ...
    def loop(self):
        ...
        self.wpnts_scaled_msg = rospy.wait_for_message(
            "global_waypoints_scaled", WpntArray)
        self.wpnts_updated_msg = self.wpnts_scaled_msg
        self.global_wpnts_msg = rospy.wait_for_message(
            "global_waypoints", WpntArray)
        self.max_speed_scaled = max([self.global_wpnts_msg.wpnts[i].vx_mps for i in range(
            len(self.global_wpnts_msg.wpnts))])
        self.s_points_array = np.array(
            [wpnt.s_m for wpnt in self.wpnts_updated_msg.wpnts])
