...
class GlobalPlanner:
    ...
    def __init__(self):
        self.input_path = os.path.join(RosPack().get_path(
            'stack_master'), 'config', 'gb_optimizer')
        ...
        self.safety_width_sp = rospy.get_param(
            '/global_planner/safety_width_sp')
        self.occupancy_grid_threshold = rospy.get_param(
            '/global_planner/occupancy_grid_threshold')

        self.show_plots = rospy.get_param(
            '/global_planner/show_plots')  # show no plots if False

        self.map_editor = rospy.get_param('/global_planner/map_editor')
        self.map_editor_mapping = rospy.get_param(
            '/global_planner/map_editor_mapping', False)
        ...
        self.reverse_mapping = rospy.get_param(
            '/global_planner/reverse_mapping', False)
        ...
        # shortest path
        self.wpnt_global_sp_pub = rospy.Publisher(
            'global_waypoints/shortest_path', WpntArray, queue_size=10)
        self.vis_wpnt_global_sp_pub = rospy.Publisher(
    ...
    def pose_cb(self, data):
        ...
        if self.lap_count == 0:
            if self.cent_driven is None:
                ...
                self.cent_driven = np.append(
                    self.cent_driven, [self.current_position], axis=0)
    ...
    def global_plan_loop(self):
        ...
        if self.map_editor and not self.map_editor_mapping:
            ...
            with open(os.path.join(self.map_dir, self.map_name + '.yaml')) as f:
                ...
                self.initial_position = [
                    self.map_origin.position.x, self.map_origin.position.y, 0]
            # compute global trajectory from img only and return
            self.compute_global_trajectory(
                cent_length=0.0, save_map=False, save_pf_copy=False)
            pb_dir = os.path.join(self.map_dir, self.map_name + '.pbstream')
            rospack = RosPack()
            script_dir = os.path.join(rospack.get_path(
                'gb_optimizer'), 'scripts/finish_map.sh')
            subprocess.Popen(args=[script_dir, pb_dir], shell=False)
            rospy.loginfo(
                '[GB Planner]: Successfully created map! Killing nodes...')
        ...
        while not rospy.is_shutdown():
            ...
            if self.map_editor_mapping and self.create_map:
                ...
                rospy.logwarn(
                    "[GB Planner]: Press 'y' when the map looks good enough")
                while True:
                    # waiting for y
                    self.current_key = input().lower().strip()
                    if self.current_key == 'y':
                        # use position after mapping is done
                        self.initial_position = self.current_position
                        self.just_once = True
                        # Save map png and yaml in map_dir, but it will break before computing the global trajectory
                        self.compute_global_trajectory(
                            cent_length=0.0, save_map=True, save_pf_copy=True)
                        pb_dir = os.path.join(
                            self.map_dir, self.map_name + '.pbstream')
                        rospack = RosPack()
                        script_dir = os.path.join(rospack.get_path(
                            'gb_optimizer'), 'scripts/finish_map.sh')
                        subprocess.Popen(
                            args=[script_dir, pb_dir], shell=False)
            # Normal mode
            else:
                ...
                if is_at_init_pos and not self.was_at_init_pos:
                    self.was_at_init_pos = True
                    self.lap_count += 1
                    rospy.loginfo("[GB Planner]:" +
                                  f"Laps completed {self.lap_count}")
                ...
                if not self.just_once and self.lap_count == self.required_laps:
                    if self.required_laps != 0:
                        ...
                        cent_length = np.sum(
                            np.sqrt(np.sum(np.power(np.diff(self.cent_driven[:, :2], axis=0), 2), axis=1)))

                        cent_len_str = "[GB Planner]:" + \
                            f"Approximate centerline length: {round(cent_length, 4)}m; "
                    ...
                    while True:
                        ...
                        if self.current_key == 'y':
                            ...
                            if self.compute_global_trajectory(cent_length=cent_length, save_map=True, save_pf_copy=True):
                                rospy.loginfo(
                                    '[GB Planner]: Successfully computed waypoints!')
                                ...
                                rospy.logwarn(
                                    '[GB Planner]: Was unable to compute waypoints in compute_global_trajectory!')
                                self.current_key = ''
                                break
                        else:
                            self.current_key = ''
    ...
    def at_init_pos_check(self) -> bool:
        ...
        theta_diff0 = math.fabs(
            self.current_position[2] - self.initial_position[2])
        theta_diff1 = 2*np.pi-theta_diff0
        theta_diff = min(theta_diff0, theta_diff1)

        at_init_pos = (x_diff < self.x_max_diff) and (
            y_diff < self.y_max_diff) and (theta_diff < self.theta_max_diff)
        return at_init_pos
    ...
    def compute_global_trajectory(self, cent_length, save_map: bool, save_pf_copy: bool) -> bool:
        ...
        if self.create_map:
            ...
            og_map = np.int8(self.map_occupancy_grid).reshape(
            ...
            opening = cv2.morphologyEx(
                bw, cv2.MORPH_OPEN, kernel1, iterations=2)
            ...
            if self.map_editor:
                ...
                while True:
                    try:
                        ...
                        msg_string = '[GB Planner]:' + \
                            f'The folder {self.map_dir} already exists!'
                        rospy.logwarn(msg_string)
                        count += 1
                        self.map_dir = map_dir_copy + str(count)
                    else:
                        self.map_name = self.map_name + \
                            str(count) if count != 0 else self.map_name
                        msg_string = '[GB Planner]:' + \
                            f'Successfully created the folder {self.map_dir}'
                ...
                if save_map:
                    img_path = os.path.join(
                        self.map_dir, self.map_name + '.png')
                ...
                ros_info_string = '[GB Planner]:' + \
                    f'PNG and YAML file created and saved in the {self.map_dir} folder'
        ...
        try:
            centerline = self.extract_centerline(
                skeleton=skeleton, cent_length=cent_length)
        except IOError:
            self.just_once = False
            if self.map_editor:
                rospy.logwarn(
                    '[GB Planner]: No closed contours found! Check the edited image...')
            else:
                rospy.logwarn(
                    '[GB Planner]: No closed contours found! Keep driving...')
            return False
        except ValueError:
            rospy.logwarn(
                "[GB Planner]: Couldn't find a closed contour with similar length as driven path!")
            rospy.loginfo('[GB Planner]: Maybe missed a lap completion...')
            rospy.loginfo(
                '[GB Planner]: Will try again in one lap, so drive at least one more lap!')
            self.just_once = False
            self.cent_length_done = False
            self.was_at_init_pos = True
            self.initial_position = self.current_position
        ...
        centerline_meter[:, 0] = centerline_smooth[:, 0] * \
            self.map_resolution + self.map_origin.position.x
        centerline_meter[:, 1] = centerline_smooth[:, 1] * \
            self.map_resolution + self.map_origin.position.y
        ...
        if self.show_plots and not self.map_editor:
            ...
            print("Direction of the initial car position: ",
                  self.initial_position[2])
            plt.plot(centerline_meter_int[:, 0], centerline_meter_int[:,
                     1], 'ko', label='Centerline interpolated')
        ...
        if self.watershed:
            try:
                bound_r_water, bound_l_water = self.extract_track_bounds(
                    centerline_smooth, opening, save_img=(not self.map_editor))
                # bound_r_water_rev, bound_l_water_rev = self.extract_track_bounds(centerline_smooth_reverse, opening)
                dist_transform = None
                rospy.loginfo(
                    '[GB Planner]: Using watershed for track bound extraction...')
            except IOError:
                rospy.logwarn(
                    '[GB Planner]: More than two track bounds detected with watershed algorithm')
                rospy.loginfo(
                    '[GB Planner]: Trying with simple distance transform...')
            ...
            rospy.loginfo(
                '[GB Planner]: Using distance transform for track bound extraction...')
            bound_r_water = None
            bound_l_water = None
            # bound_r_water_rev = None
            # bound_l_water_rev = None
        ...
        centerline_waypoints, centerline_markers = self.write_centerline(
            cent_with_dist)
        ...
        rospy.loginfo(
            '[GB Planner]: Start Global Trajectory optimization with iterative minimum curvature...')
        ...
        bounds_markers = self.publish_track_bounds(
            bound_r_iqp, bound_l_iqp, reverse=False)
        ...
        rospy.loginfo(
            '[GB Planner]: Done with iterative minimum curvature optimization')
        rospy.loginfo(
        ...
        rospy.loginfo(
            '[GB Planner]: Start Global Trajectory optimization with iterative minimum curvature for overtaking...')
        ...
        _, new_centerline_markers = self.write_centerline(
            new_cent_with_dist, sp_bool=True)
        ...
        # variable which will be published and used in l1_param_optimizer
    ...
    def extract_centerline(self, skeleton, cent_length: float) -> np.ndarray:
        ...
        contours, hierarchy = cv2.findContours(
            skeleton, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_NONE)
        ...
        # print(line_lengths)
        smallest_contour = np.array(
            closed_contours[min_length_index]).flatten()
    ...
    def smooth_centerline(centerline: np.ndarray) -> np.ndarray:
        ...
        centerline2 = np.append(
            centerline[cen_len:], centerline[0:cen_len], axis=0)
        centerline_smooth2 = savgol_filter(
            centerline2, filter_length, 3, axis=0)

        # take points from second (smoothed) centerline for first centerline
        centerline_smooth[0:filter_length] = centerline_smooth2[cen_len:(
            cen_len + filter_length)]
        centerline_smooth[-filter_length:] = centerline_smooth2[(
            cen_len - filter_length):cen_len]
    ...
    def extract_track_bounds(self, centerline: np.ndarray, filtered_bw: np.ndarray, save_img=False):
        ...
        cent_img = np.zeros(
            (filtered_bw.shape[0], filtered_bw.shape[1]), dtype=np.uint8)
        cv2.drawContours(
            cent_img, [centerline.astype(int)], 0, 255, 2, cv2.LINE_8)
        ...
        for label in np.unique(labels):
            ...
            if self.create_map and save_img:
                ...
                try:
                    with open(os.path.join(self.map_dir, self.map_name + ".yaml"), 'w') as file:
                        documents = yaml.dump(
                            dict_map, file, default_flow_style=False)
                except IOError as exp:
                    rospy.logwarn(
                        '[GB Planner]: Could not create a yaml file for simulator map')
                    print(exp)
                else:
                    sim_info_str = '[GB Planner]:' + \
                        f' Map for simulation done! Stored in {self.map_dir}'
            ...
            if len(closed_contours) != 2:
                raise IOError(
                    "More than two track bounds detected! Check input")
        ...
        bound_long_meter[:, 0] = bound_long[:, 0] * \
            self.map_resolution + self.map_origin.position.x
        bound_long_meter[:, 1] = bound_long[:, 1] * \
            self.map_resolution + self.map_origin.position.y
        ...
        bound_short_meter[:, 0] = bound_short[:, 0] * \
            self.map_resolution + self.map_origin.position.x
        bound_short_meter[:, 1] = bound_short[:, 1] * \
            self.map_resolution + self.map_origin.position.y
        ...
        if self.show_plots and not self.map_editor:
            ...
            ax1.plot(bound_right[:, 0], bound_right[:, 1],
                     'b', label='Right bound')
            ax1.plot(bound_left[:, 0], bound_left[:, 1],
                     'g', label='Left bound')
    ...
    def dist_to_bounds(self, trajectory: np.ndarray, bound_r, bound_l, centerline: np.ndarray, reverse=False):
        ...
        bound_r_tmp = np.column_stack(
            (bound_r, np.zeros((bound_r.shape[0], 2))))
        bound_l_tmp = np.column_stack(
            (bound_l, np.zeros((bound_l.shape[0], 2))))
        ...
        # contains (min) distances between waypoints and right bound
        dists_right = np.zeros(n_wpnt)
        # contains (min) distances between waypoints and left bound
        ...
        if self.show_plots and trajectory_str == "Global Trajectory":
            width_veh_real = 0.3
            normvec_normalized_opt = tph.calc_normal_vectors.calc_normal_vectors(
                trajectory[:, 3])

            veh_bound1_virt = trajectory[:, 1:3] + \
                normvec_normalized_opt * self.safety_width / 2
            ...
            ax.plot(bound_r[:, 0], bound_r[:, 1], "k-",
                    linewidth=1.0, label="Track bounds")
            ax.plot(bound_l[:, 0], bound_l[:, 1], "k-", linewidth=1.0)
            ax.plot(centerline[:, 0], centerline[:, 1],
                    "k--", linewidth=1.0, label='Centerline')

            ax.plot(veh_bound1_virt[:, 0], veh_bound1_virt[:, 1],
                    "b", linewidth=0.5, label="Vehicle width with safety")
            ax.plot(veh_bound2_virt[:, 0],
                    veh_bound2_virt[:, 1], "b", linewidth=0.5)
            ax.plot(veh_bound1_real[:, 0], veh_bound1_real[:, 1],
                    "c", linewidth=0.5, label="Real vehicle width")
            ax.plot(veh_bound2_real[:, 0],
                    veh_bound2_real[:, 1], "c", linewidth=0.5)

            ax.plot(trajectory[:, 1], trajectory[:, 2], 'tab:orange',
                    linewidth=2.0, label="Global trajectory")
    ...
    def create_wpnts_markers(self, trajectory: np.ndarray, d_right: np.ndarray, d_left: np.ndarray,
        ...
        for i, pnt in enumerate(trajectory):
            ...
            if second_traj:
                ...
                global_marker.type = global_marker.CUBE
                global_marker.scale.x = 0.15
                global_marker.scale.y = 0.15
                global_marker.scale.z = 0.15
                global_marker.color.r = 1.0
                global_marker.color.g = 1.0
                global_marker.color.b = 0.0
            else:
                # Racing line - Blue spheres
                global_marker.type = global_marker.SPHERE
                global_marker.scale.x = 0.12
                global_marker.scale.y = 0.12
                global_marker.scale.z = 0.12
