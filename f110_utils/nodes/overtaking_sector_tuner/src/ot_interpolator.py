...
class OvertakingInterpolator:
    ...
    def __init__(self, debug_plot: bool = False) -> None:
        ...
        self.spline_len = int(rospy.get_param(
            "/ot_interpolator/spline_len", 30))

        # SUBSCRIBE
        self.glb_wpnts_name = "/global_waypoints_scaled"
        self.glb_wpnts_sp_name = "/global_waypoints/shortest_path"
        rospy.Subscriber(
            "/ot_dyn_sector_server/parameter_updates", Config, self.dyn_param_cb)
        rospy.Subscriber("/dyn_sector_server/parameter_updates",
                         Config, self.dyn_speed_param_cb)
        ...
        rospy.Subscriber(self.glb_wpnts_sp_name,
                         WpntArray, self.glb_wpnts_sp_cb)
        ...
        # new glb_waypoints
        self.ot_points_pub = rospy.Publisher(
            "/global_waypoints/overtaking", WpntArray, queue_size=10)
        self.ot_markers_pub = rospy.Publisher(
            "/global_waypoints/overtaking/markers", MarkerArray, queue_size=10)
    ...
    def glb_wpnts_sp_cb(self, data: WpntArray):
        ...
        coords = np.array(
            [[wpnt.s_m, wpnt.x_m, wpnt.y_m, wpnt.vx_mps]
                for wpnt in data.wpnts]
        )
        ...
        for i in range(self.n_sectors):
            ...
            if self.sectors_params[sector]['ot_flag']:
                ...
                if self.next_sector_overtaking(i):
                    schedule[self.sectors_params[sector]['end'] -
                             switching_sec_len+1:self.sectors_params[sector]['end']+1] = 1
                else:
                    for j in range(1, switching_sec_len+1):
                        schedule[self.sectors_params[sector]['end']-j] = self.interpolating_function(
                            j, switching_sec_len, mode='sigmoid')

                schedule[self.sectors_params[sector]['start']+switching_sec_len -
                         1:self.sectors_params[sector]['end']-switching_sec_len+1] = 1
    ...
    def recalculate_ot_s(self):
        ...
        for wpnt in self.interp_wpnt.wpnts:
            if i > 0:
                ...
                dist_from_previous = np.linalg.norm(
                    [wpnt.x_m - coords[i-1, 1], wpnt.y_m - coords[i-1, 2]])
                ...
                i += 1
        tot_len = s + \
            np.linalg.norm([coords[0, 1] - coords[i-1, 1],
                           coords[0, 2] - coords[i-1, 2]])
        ...
        for i in range(0, int(tot_len*10)):
            ...
            # frenet components
            frenet_newpoint = self.converter.get_frenet(
                np.array([new_wpnt.x_m]), np.array([new_wpnt.y_m]))
    ...
    def publish_markers(self):
        ...
        for i, wpnt in enumerate(self.interp_wpnt.wpnts):
            ...
            # Alternative racing line - Dark green arrows for better visibility
            global_marker.type = global_marker.ARROW
            global_marker.scale.x = 0.2  # Length
            global_marker.scale.y = 0.05  # Width
            global_marker.scale.z = 0.05  # Height
            global_marker.color.a = 1.0
            global_marker.color.r = 0.0
            global_marker.color.g = 0.8  # Brighter green for better visibility
            global_marker.color.b = 0.0

            global_marker.id = i
            global_marker.pose.position.x = wpnt.x_m
            global_marker.pose.position.y = wpnt.y_m
            global_marker.pose.position.z = 0.1  # Slight elevation for visibility
            # Set orientation based on heading
            global_marker.pose.orientation.z = math.sin(wpnt.psi_rad / 2)
            global_marker.pose.orientation.w = math.cos(wpnt.psi_rad / 2)
